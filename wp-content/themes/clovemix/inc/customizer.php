...
function clovemix_sanitize_checkbox( $input ) {
    if ( $input == 1 ) {
        return 1;
    } else {
        return '';
...
//Add a class for titles
	...
	$wp_customize->add_section(
        ...
        array(
            ...
            'priority' => 1,
            'description' => __('<strong>Logo and favicon Settings available in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'" target="_blank">PRO Version</a>.',
        )
    );  
	...
	$wp_customize->add_section('opacity',array(
			...
			'description'	=> __('<strong>Background opacity available in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'">PRO Version</a>',
			'priority'	=> 2
	));
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control($wp_customize,'color_scheme',array(
			...
			'description'	=> __('<strong>More color options in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'" target="_blank">PRO version</a>',
			'section' => 'colors',
			'settings' => 'color_scheme'
		))
	);
	
	$wp_customize->add_setting('color_copy', array(
		'default' => '#2d2a28',
		'sanitize_callback'	=> 'sanitize_hex_color',
	));
	
	$wp_customize->add_control(
		new WP_Customize_Color_Control($wp_customize,'color_copy',array(
			'description'	=> __('Add background color for footer area','clovemix'),
			'section' => 'colors',
			'settings' => 'color_copy'
		))
	);
	...
	$wp_customize->add_section('social_section',array(
		...
		'description'	=> __('Add your social links here. <br><strong>More social links in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'" target="_blank">PRO version</a>.',
		'priority'		=> null
	));
	...
	$wp_customize->add_section('slider_section',array(
		...
		'description'	=> __('Add slider images here. <br><strong>More slider settings available in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'" target="_blank">PRO version</a>.',
		'priority'		=> null
	));
	
	$wp_customize->add_setting('caption_hide',array(
		'sanitize_callback'	=> 'clovemix_sanitize_checkbox'
	));
	
	$wp_customize->add_control('caption_hide',array(
			'label'	=> __('Check this to hide slide caption','clovemix'),
			'setting'	=> 'caption_hide',
	...
	$wp_customize->add_section('home_sec',array(
			...
			'description'	=> __('<strong>Homepage sections available in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'">PRO Version</a>',
			'priority'	=> null
	));	
	...
	$wp_customize->add_section(
        ...
        array(
            ...
            'priority' => null,
            'description' => __('<strong>Layout Settings available in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'" target="_blank">PRO Version</a>.',
        )
    );  
	...
	$wp_customize->add_section(
        ...
        array(
            ...
            'priority' => null,
            'description' => __('<strong>Font Settings available in</strong>','clovemix'). '<a href="'.esc_url(pro_theme_url).'" target="_blank">PRO Version</a>.',
        )
    );  
    $wp_customize->add_setting('Clovemix_options[font-info]', array(
			'sanitize_callback' => 'sanitize_text_field',
    ...
    $wp_customize->add_section(
        ...
        array(
            ...
            'priority' => null,
            'description' => __('For documentation and support check this link :','clovemix'). '<a href="'.esc_url(theme_doc).'" target="_blank">Clovemix Documentation</a>',
        )
    );  
...
function clovemix_css(){
        ...
        <style>
				...
				.woocommerce ul.products li.product .price{
					color:<?php echo get_theme_mod('color_scheme','#00d603'); ?>;
				}
				...
				form.search-form input[type="submit"]:hover{
					background-color:<?php echo get_theme_mod('color_scheme','#00d603'); ?>;
				}
				.nivo-caption{<?php if(get_theme_mod('caption_hide') != '') {?> display:none !important; <?php } ?>}
				#footer{ background-color:<?php echo get_theme_mod('color_copy','#2d2a28'); ?>;}
		</style>
	<?php }
